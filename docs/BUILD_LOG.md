...
- 🟡 **Backstage-first build order** (per Louis) — pure logic + correctness first (no cron/Telegram wiring yet); frontstage wiring is a deliberate later pass. Next: Decision History + single-writer rule → currency tags + minimal C layer → A3 → A4 → A1/A5/B5 run modes.
